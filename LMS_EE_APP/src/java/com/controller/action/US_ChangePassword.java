...
public class US_ChangePassword extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (request.getSession().getAttribute("user") != null) {
                ...
                String URL = ResAccessUrl.USER_SERVICE + "ChangePassword/" + obj.getInt("loginID") + "/" + cpw + "/" + npw + "/" + rpw;
                String METHOD = "PUT";
                String res = new ResAccessMethod().callMethod(URL, METHOD);

                if (res.isEmpty()) {
                    response.sendRedirect("user_profile.jsp?error=Password change failed");
                } else {
                    JSONObject obj2 = new JSONObject(res);
                    if (obj2.getBoolean("result")) {
                        response.sendRedirect("user_profile.jsp?error=Successfully changed");
                    } else {
