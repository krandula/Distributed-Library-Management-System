...
public class BS_UpdateBook extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (request.getSession().getAttribute("user") != null) {
                ...
                String URL = ResAccessUrl.BOOK_SERVICE + "BookProfile/" + lhid + "/"+book+"/" + name + "/" + isbn + "/" + description+"/"+bookCount+"/"+printedYear+"/"+author+"/"+category+"/"+printers;
                String METHOD = "PUT";
                String res = new ResAccessMethod().callMethod(URL, METHOD);

                if (res.isEmpty()) {
                    response.sendRedirect("book_management.jsp?error=New book update failed");
                } else {
                    JSONObject obj2 = new JSONObject(res);
                    if (obj2.getBoolean("result")) {
                        response.sendRedirect("book_management.jsp?error=Successfully updated");
                    } else {
